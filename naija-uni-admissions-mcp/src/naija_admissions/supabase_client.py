"""Supabase client for Campus Compass knowledge base.

Provides:
- Async client with connection pooling
- Upsert operations for all knowledge tables
- Crawl logging with timestamps, confidence, status
- Staging layer (raw_crawl_data, validated_data) for validation before production
"""
...
try:
    ...
    # Load from project root (naija-uni-admissions-mcp)
...
def _get_config() -> SupabaseConfig:
    """Get config lazily - reads env vars at call time."""
...
async def get_client(use_service_role: bool = False) -> AsyncClient:
    ...
    if use_service_role:
        if _service_client is None:
            cfg = _get_config()
            if not cfg.validate():
                raise RuntimeError("Supabase configuration incomplete. Check SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY in .env")
...
def _generate_content_hash(data: dict) -> str:
    ...
    # Normalize: sort keys, remove timestamp fields
...
# =============================================================================
# PRODUCTION TABLES UPSERT FUNCTIONS
# =============================================================================
...
async def upsert_faculty(
    ...
    """Upsert faculty. Returns the upserted row."""
    client = await get_client(use_service_role=True)
...
async def upsert_program(
    ...
    """Upsert program. Returns the upserted row."""
    client = await get_client(use_service_role=True)
...
async def upsert_admission_requirements(
    ...
    """Upsert admission requirements. Returns the upserted row."""
    client = await get_client(use_service_role=True)
...
async def upsert_source_document(
    ...
    """Upsert source document for provenance tracking."""
    client = await get_client(use_service_role=True)
...
async def log_crawl(
    ...
    """Log a crawl attempt to crawl_logs table."""
    client = await get_client(use_service_role=True)
    
    # Find institution_id if name provided
    if institution_id is None and institution_name:
        inst_result = await run_with_retry(
            client.table("institutions").select("id").eq("name", institution_name).limit(1).execute()
        )
        if inst_result.data:
            institution_id = inst_result.data[0]["id"]
...
# =============================================================================
# STAGING LAYER (raw_crawl_data, validated_data)
# =============================================================================
...
async def stage_raw_crawl(
    ...
    """Store raw crawl data in staging table before validation."""
    client = await get_client(use_service_role=True)
...
async def promote_to_validated(
    ...
    await run_with_retry(
        client.table("raw_crawl_data").update({"status": "validated"}).eq("id", raw_crawl_id).execute()
    )
...
# =============================================================================
# HIGH-LEVEL INTEGRATION: FULL INSTITUTION UPSERT
# =============================================================================
...
async def upsert_full_institution(
    ...
    Returns dict with all created/updated record IDs.
    """
    results = {}
    ...
    # 4. Upsert admission requirements
    if admission_reqs:
        req = await upsert_admission_requirements(
            institution_id=inst_id,
            program_id=None,  # institution-level
            **admission_reqs,
        )
        if req:
            results["admission_requirements_id"] = req["id"]
    
    # 5. Upsert program-level requirements if provided
